...
const BASE22: &[u8] = b"BCDFGHJKMPQRTVWXY2346789";
            ...
            Some(
                [key.chars().filter(|&c| c != '-').position(|c| c == 'N')? as u128]
                    ...
                    .chain(key.chars().filter_map(|c| {
                        if c != 'N' && c != '-' {
                            Some(BASE22.iter().position(|&b| b as char == c)? as u128)
                        } else {
                            None
                        }
                    }))
...
impl ProductKeyEncoder {
    ...
    pub fn new(group: u128, serial: u128, security: u128, upgrade: bool, checksum: Option<u128>, extra: bool) -> Option<Self> {
        let upgrade = if upgrade { 1 } else { 0 };
        let extra = if extra { 1 } else { 0 };
        if Self::BOUNDS.iter().zip([group, serial, security, checksum.unwrap_or(1) - 1, upgrade, extra]).all(|(&bound, value)| {
            value <= bound
        }) {
            let mut raw_key = 0;
            raw_key |= extra << 114;
            ...
            if extra != 0 && raw_key > (0x62A32B15518 << 72) { return None }
            Some(Self {
    ...
    pub fn to_5x5(mut raw_key: u128) -> String {
        let mut key = (0..25).map(|_| { let v = raw_key % 24; raw_key /= 24; v }).collect::<Vec<_>>();
        key.reverse();
        let mut product_key = key.iter().skip(1).fold(String::new(), |mut s, &v| { s.push(BASE22[v as usize] as char); s });
        product_key.insert(key[0] as usize, 'N');
        product_key.insert(5,  '-');
        product_key.insert(11, '-');
        product_key.insert(17, '-');
        product_key.insert(23, '-');
        product_key
    }
}
...
#[test]
fn it_works() {
    assert!(ProductKeyDecoder::new("NPPR9-FWDCX-D2C8J-H872K-2YT43").unwrap() == ProductKeyEncoder::new(3290, 3, 4390648297412806, false, Some(51), false).unwrap());
}